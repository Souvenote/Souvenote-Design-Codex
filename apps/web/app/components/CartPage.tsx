...
function normalizeCartItems(raw: unknown): CartItem[] {
  ...
  raw.forEach((item, index) => {
    ...
    if (existing) {
      if (
        normalized.id === 'pack-bigsender' ||
        normalized.id === 'pack-try-risk-free' ||
        isBlankSouvenoteGiftId(normalized.id)
      ) {
        ...
        {lockedQuantity ? (
          <div className="cart-qty cart-qty-locked" aria-label={`${lockedLabel} selected`}>
            ...
            <span className="cart-qty-label">
              {item.cardCount ? (item.cardCount === 1 ? 'card' : 'cards') : 'gift'}
            </span>
          </div>
        ) : (
...
function GiftCheckoutOffer({ open, onClose, onDecline, onAdd }: GiftCheckoutOfferProps) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="cart-gift-offer-title"
      data-screen-label="Cart · Gift add-on"
    >
      ...
      <div className="bmc-modal cart-gift-modal">
        ...
        <div className="cart-gift-layout">
          <div className="cart-gift-copy">
            ...
            <h2 id="cart-gift-offer-title" className="cart-gift-title">
              Add a blank <span className="souv-hero-italic text-metallic-gold">Souvenote</span> to your order for
              $6.99?
            </h2>
            <p className="cart-gift-lede">
              This lets a recipient of your choice create a Souvenote for someone else. We&apos;ll remind you at
              delivery that you have one to give!
            </p>
            <div className="cart-gift-includes">
              <div className="acc-gift-inc">
                <span className="acc-gift-inc-ico">
                  <CartIco name="spark" w={20} />
                </span>
                <div>
                  <div className="acc-gift-inc-h">Blank card experience</div>
                  <div className="acc-gift-inc-p">They choose the recipient, message, and creative direction.</div>
                </div>
              </div>
              <div className="acc-gift-inc">
                <span className="acc-gift-inc-ico">
                  <CartIco name="truck" w={20} />
                </span>
                <div>
                  <div className="acc-gift-inc-h">Delivery reminder</div>
                  <div className="acc-gift-inc-p">We&apos;ll surface the gift again when you send your card.</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <aside className="acc-panel acc-gift-summary cart-gift-summary">
            ...
            <div className="acc-gift-meta">
              <div className="acc-summary-row">
                ...
                <span className="v">1 blank gift</span>
              </div>
              <div className="acc-summary-row">
                <span className="k">Recipient</span>
                <span className="v">Chosen at delivery</span>
              </div>
              <div className="acc-summary-row">
        ...
        <div className="cart-gift-actions">
          <button type="button" className="bmc-cta-secondary cart-gift-skip" onClick={onDecline}>
            No thanks
          </button>
          <button type="button" className="bmc-cta cart-gift-add" onClick={onAdd}>
            Add gift · $6.99 <CartIco name="arrow" w={15} />
          </button>
        </div>
      </div>
    </div>
...
function CartPage() {
  ...
  async function handlePaid() {
    ...
    const blankGiftCount = items.reduce(
      (sum, item) => (isBlankSouvenoteGiftId(item.id) ? sum + Math.max(1, item.qty) : sum),
      0,
    );
  ...
  const checkoutPack: CheckoutPack = lockedCartItem
    ? {
        kind: 'cards',
        name: lockedCartItem.name,
        price: subtotal,
        cards: lockedCardCount,
        tokens: lockedCardCount * lockedCreditsPerCard,
        bonus: 0,
      }
