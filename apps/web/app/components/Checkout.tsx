      ...
      <div className="bmc-modal co-picker">
        ...
        <div className="co-picker-head">
          <div
            ...
            style={{ justifyContent: 'center', color: 'var(--rose-gold)', whiteSpace: 'nowrap' }}
          >
          ...
          <div className="co-picker-tabs">
            <div className="bmc-chip-row">
              <button
                type="button"
                className={`bmc-chip ${tab === 'cards' ? 'is-active' : ''}`}
              ...
              <button
                type="button"
                className={`bmc-chip ${tab === 'credits' ? 'is-active' : ''}`}
        ...
        <div className="co-pk-grid">
          {packs.map((pack) => (
            <div key={pack.id} className={`co-pk ${pack.featured ? 'is-featured' : ''}`}>
              ...
              <div className="co-pk-count-label">
                {tab === 'cards' ? (pack.cards === 1 ? 'Card' : 'Cards') : 'Credits'}
              </div>
              <div className="co-pk-price">
                {coMoney(pack.price)}
                <span className="co-total-cur"> CAD</span>
              </div>
...
function CheckoutModal({ open, pack, country = 'CA', onClose, onPaid, onBack }: CheckoutModalProps) {
  ...
  function pay() {
    ...
    setPaymentError(null);
    window.setTimeout(() => {
      Promise.resolve(onPaid?.(pack)).catch((error) => {
        setPaymentError(
          error instanceof Error
            ? error.message
            : 'Payment succeeded, but credits could not be added. Please try again.',
        );
        setProcessing(false);
      });
    }, 1300);
        ...
        <div className="co-grid">
          <div className="co-pay">
            <div className="co-eyebrow">
              <BmcIcon name="lock" w={13} /> Secure checkout
            </div>
            ...
            <div className="co-card-el">
              <div>
                ...
                <div className="co-input-wrap">
                  <input
                    ...
                    inputMode="numeric"
                    value={card.number}
                    onChange={(event) => setCard({ ...card, number: fmtCard(event.target.value) })}
              ...
              <div className="co-card-split">
                <div>
                  ...
                  <input
                    ...
                    inputMode="numeric"
                    value={card.exp}
                    onChange={(event) => setCard({ ...card, exp: fmtExp(event.target.value) })}
                    placeholder="MM / YY"
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <input
                  className="co-input"
                  value={card.postal}
                  onChange={(event) => setCard({ ...card, postal: event.target.value })}
                  placeholder="V6B 1A1"
                />
              </div>
            </div>
            ...
            <div style={{ marginTop: 18 }}>
              ...
              {promoApplied ? (
                <div className="co-promo-ok">
                  <BmcIcon name="check" w={13} /> Code <b style={{ fontStyle: 'normal' }}>SOUVENOTE10</b> applied, 10%
                  off
                </div>
              ) : (
            ...
            <button
              ...
              onClick={pay}
              style={processing ? { opacity: 0.7, pointerEvents: 'none' } : undefined}
            >
              {processing ? (
                <>
                  <span
                    className="bmc-gen-spin"
                    style={{ borderTopColor: '#2a1015', borderColor: 'rgba(42,16,21,0.3)' }}
                  />{' '}
                  Processing...
                </>
              ) : (
                <>
                  Pay {coMoney(total)} CAD <BmcIcon name="arrow" w={15} />
                </>
              )}
            </button>
            {paymentError && (
              <div
                className="co-promo-ok"
                role="alert"
                style={{ marginTop: 12, borderColor: 'rgba(229,184,177,.45)', color: 'var(--rose-gold-hi)' }}
              >
                {paymentError}
              </div>
            )}
            <div className="co-secure">
              <BmcIcon name="lock" w={13} /> Payments secured by <b>Stripe</b>
            </div>
            {onBack && (
              <button
                type="button"
          ...
          <div className="co-summary">
            ...
            <div className="co-pack">
              ...
              <div className="co-pack-tag">
                {isCart ? (
                  <>
                    <b>
                      {pack.lineCount} {pack.lineCount === 1 ? 'item' : 'items'}
                    </b>{' '}
                    · cards, packs &amp; credits
                  </>
                ) : isCards ? (
                  <>
                    ...
                    · {pack.cards === 1 ? 'one physical send' : 'physical sends'} · shipping included
                  </>
                ) : (
                  <>
            ...
            <div className="co-lines">
              <div className="co-line">
                <span className="co-line-label">Subtotal</span>
                <span className="co-line-val">{coMoney(subtotal)}</span>
              </div>
              {(isCards || isCart) && (
                <div className="co-line">
                  <span className="co-line-label">Shipping</span>
                  <span className="co-line-val is-discount">Included</span>
                </div>
              )}
              {promoApplied && (
                <div className="co-line">
                  <span className="co-line-label">Promo · SOUVENOTE10</span>
                  <span className="co-line-val is-discount">
                    {'\u2212'}
                    {coMoney(discount)}
                  </span>
                </div>
              ...
              {tax > 0 && (
                <div className="co-line">
                  <span className="co-line-label">{taxLabel}</span>
                  <span className="co-line-val">{coMoney(tax)}</span>
                </div>
              )}
            </div>
            ...
            <div className="co-total">
              ...
              <span>
                <span className="co-total-val">{coMoney(total)}</span>
                <span className="co-total-cur">CAD</span>
              </span>
            </div>
            ...
            {isCards && Number(pack.bonus) > 0 && (
              <div className="co-bonus">
                <BmcIcon name="spark2" w={16} /> Includes {pack.tokens} AI credits, plus {pack.bonus} bonus on your
                first send
              </div>
            )}
            <p className="co-fx">
              {taxLabel === 'Tax'
                ? 'Tax calculated at your delivery address.'
                : `${taxLabel.split(' ')[0]} calculated from your delivery address.`}{' '}
              Cards never expire for 12 months.
            </p>
          </div>
        </div>
      </div>
