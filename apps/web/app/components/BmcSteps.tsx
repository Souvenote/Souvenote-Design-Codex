  ...
  return (
    <>
      <BmcHead
        ...
        italicAccent={false}
        titleStyle={{
          fontFamily: "var(--font-display, 'Outfit'), ui-sans-serif, system-ui, sans-serif",
          fontSize: '53px',
          lineHeight: 1.02,
          fontStyle: 'normal',
      ...
      <div className="bmc-photo-grid">
        ...
        >
          <span className="bmc-photo-drop-icon">
            <BmcIcon name="upload" w={32} />
          </span>
        ...
        <div className={`bmc-photo-or ${describe ? 'is-active' : ''}`}>
          ...
          <p className="bmc-photo-or-sub">
            Describe a memory, joke, or imaginary scene. We'll generate it from your words alone.
          </p>
          <button type="button" className={describe ? 'bmc-cta' : 'bmc-cta-secondary'} onClick={openDescribeSection}>
      ...
      {describe && (
        <div className="bmc-describe-inline">
          <label className="bmc-label bmc-describe-label" htmlFor="bmc-describe-input">
            Describe your card
          </label>
          <textarea
          ...
          <div className="bmc-ref-thumbs">
            {files.map((f, i) => (
              <span key={i} className="bmc-ref-thumb" style={{ backgroundImage: `url(${f.url})` }}>
                <button type="button" className="bmc-ref-x" onClick={() => removeAt(i)} aria-label="Remove">
                  <BmcIcon name="close" w={12} />
                </button>
              </span>
            ))}
          </div>
      ...
      {hasFiles && isCA && (
        <div className={`bmc-attest-gate ${attested ? 'is-done' : ''}`}>
          <span className="bmc-attest-gate-icon">
            <BmcIcon name={attested ? 'check' : 'lock'} w={18} />
          </span>
          <div className="bmc-attest-gate-text">
            {attested ? (
              ...
              <>
                <b>Image rights required.</b> Because you uploaded a likeness, review our Terms &amp; Privacy and
      ...
      {describe && describeTouched && !hasDescription && (
        <div className="bmc-prompt-wrap" role="dialog" aria-modal="true">
          ...
          <div className="bmc-prompt">
            <span className="bmc-prompt-icon">
              <BmcIcon name="sparkle" w={22} />
            </span>
...
function BmcBasicsStep({
  onContinue,
  onBack,
  initialDraft,
  onDraftPatch,
}: StepNavProps & { initialDraft?: Record<string, unknown>; onDraftPatch?: (patch: BmcDraftInputPatch) => void }) {
  ...
  const [occasion, setOccasion] = React.useState(
    initialOccasion && !OCCASIONS.includes(initialOccasion) ? customOccasionLabel : initialOccasion,
  );
  const [custom, setCustom] = React.useState(
    initialOccasion && !OCCASIONS.includes(initialOccasion) ? initialOccasion : '',
  );
  ...
  return (
    <>
      ...
      <div
        className="bmc-stack-lg bmc-basics"
        style={{ '--occasion-accent': OCCASION_ACCENTS[occasion] || 'var(--gold)' } as CSSVarStyle}
      >
        <div className="bmc-fieldcard">
          <label className="bmc-label">Card orientation</label>
          <div className="bmc-orient-toggle">
            <div
              className={`bmc-orient bmc-orient-portrait ${orientation === 'portrait' ? 'is-active' : ''}`}
            ...
            <div
              className={`bmc-orient bmc-orient-landscape ${orientation === 'landscape' ? 'is-active' : ''}`}
        ...
        <div className="bmc-fieldcard">
          <label className="bmc-label">
            Occasion <em className="bmc-opt">· optional</em>
          </label>
          ...
          <div className="bmc-chip-row">
            {OCCASIONS.map((o) => (
              <button
                key={o}
                type="button"
                className={`bmc-chip bmc-chip-occasion ${o === occasion ? 'is-active' : ''}`}
                style={{ '--chip-accent': OCCASION_ACCENTS[o] || 'var(--gold)' } as CSSVarStyle}
          ...
          {isCustom && (
            <input
              className="bmc-input"
              style={{ marginTop: 14 }}
              placeholder="Name your occasion, e.g. “Half-birthday”, “First apartment”"
              value={custom}
        ...
        <div className="bmc-fieldcard">
          <label className="bmc-label">
            Who it's for <em className="bmc-opt">· all optional</em>
          </label>
          <div className={`bmc-grid-2 bmc-basics-names ${skipped ? 'is-dim' : ''}`}>
            <div>
              <label className="bmc-label">
                Recipient name <em className="bmc-opt">· what do you call them?</em>
              </label>
              <input
                className="bmc-input"
                placeholder="We include this in your inside message and optional song lyrics"
                value={recipient}
            ...
            <div>
              <label className="bmc-label">
                Phonetic spelling <em className="bmc-opt">· optional</em>
              </label>
              <input
                className="bmc-input"
                placeholder="Used only as a pronunciation aid if you add a song"
                value={phonetic}
            ...
            <div>
              ...
              <input
                className="bmc-input"
                placeholder="My mother, best friend, partner…"
                value={relationship}
...
const BLUEPRINTS: Blueprint[] = [
  {
    id: 'transform',
    title: 'Transform Scene and Style',
    sub: 'Completely reimagine a photo or build your idea from scratch.',
    note: 'Works with a photo or written idea',
    popular: true,
    needsPhoto: false,
  },
  {
    id: 'enhance',
    title: 'Enhance Style',
    sub: 'Keep your photo\u2019s layout but repaint it in a new artistic style.',
    needsPhoto: true,
  },
  {
    id: 'decorate',
    title: 'Decorate My Photo',
    sub: 'Keep your original photo and add festive borders, accents, and text.',
    needsPhoto: true,
  },
];
...
function BmcImageStep({ onContinue, onBack, hasPhoto, initialDraft, onDraftPatch }: BmcImageStepProps) {
  ...
  const toggleVibe = (v: string) =>
    setVibes((curr) => (curr.includes(v) ? curr.filter((x) => x !== v) : curr.length >= 3 ? curr : [...curr, v]));

  const toggleAccent = (a: string) =>
    setAccents((curr) => {
  ...
  const tryContinue = () => {
    if (coverMode === 'with' && !coverText.trim()) {
      setCoverErr(true);
      bmcError(
        'Enter the words you’d like printed on the front of your card, or switch to “No card caption” to continue.',
        'Card caption needed',
      );
      return;
    }
          ...
          {BLUEPRINTS.map((b, i) => {
            ...
            return (
              ...
              >
                ...
                {locked ? (
                  <span className="bmc-blueprint-tag">
                    <BmcIcon name="lock" w={12} /> Upload a photo to unlock
                  </span>
                ) : b.needsPhoto ? (
                  <span className="bmc-blueprint-tag" style={{ color: 'var(--rose-gold)' }}>
                    Photo-based
                  </span>
                ) : (
      ...
      <div className="bmc-stack-lg">
        <div className="bmc-fieldcard">
          ...
          <div className="bmc-styles">
            {VISUAL_STYLES.map((s) => (
              <div
                key={s.id}
                className={`bmc-style ${s.id === style ? 'is-active' : ''}`}
          ...
          {style === 'custom' && (
            <input
              className="bmc-input bmc-custom-input"
              autoFocus
              placeholder="Describe your own visual style…"
              value={styleCustom}
          ...
          <div className="bmc-chip-row">
            {VIBES.map((v) => (
              <button
                key={v}
                type="button"
                className={`bmc-chip ${v === 'Custom\u2026' ? 'bmc-chip-custom' : ''} ${vibes.includes(v) ? 'is-active' : ''}`}
                onClick={() => toggleVibe(v)}
              >
                {vibes.includes(v) && v !== 'Custom\u2026' && <BmcIcon name="check" w={13} />}
                {v}
              </button>
            ))}
          </div>
        ...
        {blueprint === 'transform' && (
          <div className="bmc-fieldcard">
            ...
            />
            <p className="bmc-help">
              Transform Scene and Style can also inspire optional QR-song lyrics when you include a song.
            </p>
          </div>
        )}
        ...
        {(blueprint === 'enhance' || blueprint === 'decorate') && (
          <div className="bmc-fieldcard">
            <label className="bmc-label">
              Decorative accents <span className="bmc-chip-count">{accents.length} of 3</span>
            </label>
            <p className="bmc-help" style={{ margin: '-4px 0 12px' }}>
              Select up to three.
            </p>
            <div className="bmc-chip-row">
              {ACCENTS.map((a) => {
                ...
                return (
                  ...
                  >
                    {accents.includes(a) && a !== 'Custom\u2026' && <BmcIcon name="check" w={13} />}
                    {a}
                  </button>
                );
              })}
            ...
            {accents.includes('Custom\u2026') && (
              <input
                className="bmc-input bmc-custom-input"
                autoFocus
                placeholder="Describe your own accent…"
                value={accentCustom}
          ...
          <div className="bmc-fieldcard">
            ...
            <div className="bmc-chip-row">
              {BORDERS.map((b) => (
                <button
                  key={b}
                  type="button"
                  className={`bmc-chip ${b === 'Custom\u2026' ? 'bmc-chip-custom' : ''} ${b === border ? 'is-active' : ''}`}
                  onClick={() => setBorder(b)}
                >
                  {b}
                </button>
            ...
            {border === 'Custom\u2026' && (
              <input
                className="bmc-input bmc-custom-input"
                autoFocus
                placeholder="Describe your own border or design treatment…"
                value={borderCustom}
        ...
        <div className="bmc-fieldcard">
          ...
          <div className="bmc-cover-row">
            <div className="bmc-chip-row bmc-caption-actions">
              <button
                type="button"
                className={`bmc-chip ${coverMode === 'with' ? 'is-active' : ''}`}
                onClick={() => setCoverMode('with')}
              >
                Card caption
              </button>
              <button
                type="button"
                className={`bmc-chip ${coverMode === 'none' ? 'is-active' : ''}`}
                onClick={() => {
                  setCoverMode('none');
                  setCoverErr(false);
                }}
            ...
            <div>
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value.trim()) setCoverErr(false);
                }}
...
// ============================================================
function BmcMessageStep({
  onContinue,
  onBack,
  blueprintLabel = 'Transform Scene and Style',
  initialDraft,
  onDraftPatch,
}: BmcMessageStepProps) {
      ...
      <div className="bmc-stack-lg">
        <div className="bmc-fieldcard">
          ...
          <textarea
            ...
            maxLength={500}
            placeholder={
              error ? 'Add message' : 'Write the message as you want, or click "Message Generator" if you need help.'
            }
            value={msg}
            onChange={(e) => {
          ...
          <div className="bmc-row" style={{ justifyContent: 'space-between', marginTop: 10 }}>
            <span className="bmc-help" style={{ margin: 0 }}>
              1–4 short lines · printable greeting-card language.
            </span>
            <span className="bmc-msg-counter">
              <span>
                <b>{charsLeft}</b> chars left
              </span>
            </span>
          </div>
...
const GENRE_GROUPS: GenreGroup[] = [
  {
    title: 'Pop · R&B · Soul',
    genres: [
      ['1970s Motown Soul', 'Male'],
      ['Upbeat Summer Pop', 'Female'],
      ['Slow R&B Ballad', 'Male'],
      ['Modern Pop-Funk Crossover', 'Female'],
      ['Aspirational Digital R&B', 'Female'],
      ['High-Energy Tokyo Synth-Pop', 'Female'],
    ],
  },
  {
    ...
    genres: [
      ['Anthemic Rock', 'Male'],
      ['Classic Rock Anthem', 'Male'],
      ['Early 2000s Punk-Rock', 'Mixed'],
      ['Motown-Influenced Rock', 'Male'],
    ],
  },
  {
    title: 'Hip-Hop · Rap · Gospel',
    genres: [
      ['Gospel-Rap Progression', 'Male & Choir'],
      ['Boom-Bap Rap', 'Male'],
      ['Modern Trap', 'Mixed'],
      ['Gospel-Hip-Hop Fusion', 'Choir'],
    ],
  },
  {
    title: 'Jazz · Bossa Nova · World',
    genres: [
      ['Romantic Bossa Nova & R&B Fusion', 'Male & Female'],
      ['Smooth Jazz Ballad', 'Female'],
      ['West African Afrobeats', 'Mixed'],
    ],
  },
  {
    title: 'Electronic · Folk · Ambient',
    genres: [
      ['Wistful Liquid Drum & Bass', 'Female'],
      ['Narrative Indie Folk', 'Male'],
      ['Atmospheric Chill-Step', 'Female'],
    ],
  },
];
      ...
      <div className="bmc-stack-lg">
        <div className="bmc-card bmc-song-choice">
          ...
          <p className="bmc-help" style={{ marginTop: 10 }}>
            Turn this off to generate only the card image and printed message. You can still add a song later from Saved
            Cards &amp; Songs.
          </p>
        </div>
        ...
        {includeSong ? (
                ...
                <div key={group.title} className="bmc-genre-group">
                  ...
                  <div className="bmc-genres">
                    {group.genres.map(([name, voice]) => (
                      <div
                        key={name}
                        className={`bmc-genre ${name === genre ? 'is-active' : ''}`}
            ...
            <div className="bmc-card">
              <div className="bmc-card-head">
                ...
                <div className="bmc-card-meta">
                  {lyrics.length} / 750 · {remixes} of 5 regenerations
                </div>
              </div>
              ...
              <div className="bmc-lyrics-controls" style={{ marginTop: 16 }}>
                <button
                  ...
                  onClick={() => setRemixes((r) => Math.min(5, r + 1))}
                >
                ...
                {!editing && (
                  <span className="bmc-help" style={{ margin: 0 }}>
                    Lyrics are read-only until you click Edit.
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="bmc-card bmc-song-skip-card">
            ...
            <p className="bmc-help" style={{ marginTop: 8 }}>
              Your Souvenote will still include the generated image and inside message. The printed card simply ships
              without a song QR code.
            </p>
          </div>
        )}
      </div>
