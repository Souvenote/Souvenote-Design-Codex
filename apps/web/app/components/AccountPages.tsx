...
function ProfilePage({ user }: AccountPageProps) {
  ...
  const profileStats: ProfileStat[] = [
    {
      num: creditBalance.status === 'loading' ? '...' : String(creditBalance.balance),
      label: creditBalance.status === 'error' ? 'Credits offline' : 'Credits',
      gold: true,
    },
    {
      num: draftCountStatus === 'loading' ? '...' : String(cardDraftCount),
      label: draftCountStatus === 'error' ? 'Cards offline' : 'Cards made',
    },
    ...
    <div className="bmc-shell" data-screen-label="Profile">
      <div className="bmc-head" style={{ marginBottom: 28 }}>
        ...
        <h1 className="bmc-title">
          Your <span className="souv-hero-italic text-metallic-gold">profile</span>
        </h1>
        <p className="bmc-lede">
          Everything about your account at a glance: credits, what you&apos;ve made, and quick ways back into the things
          you do most.
        </p>
      </div>
          ...
          <div className="acc-activity">
            {profileActivity.length ? (
              ...
              <div className="acc-activity-row">
                ...
                <div className="acc-activity-main">
                  <div className="acc-activity-title">No activity yet. Create your first card to get started.</div>
                </div>
  ...
  return (
    <div className="bmc-shell" data-screen-label="Refer a Friend">
      ...
      <div className="acc-panel acc-referlink-wrap" style={{ marginBottom: 22 }}>
        <div className="acc-flabel" style={{ textAlign: 'center', marginBottom: 12 }}>
      ...
      <div className="acc-grid-2">
            ...
            <div className="acc-reftable">
              ...
              {REFER_LIST.map((referral) => (
                <div className="acc-reftable-row" key={referral.name}>
                  ...
                  <span className={`acc-pill ${referral.status === 'done' ? 'is-done' : 'is-pending'}`}>
        ...
        >
          <div className="acc-panel-title" style={{ borderBottom: 0, marginBottom: 6 }}>
            Credits earned
          </div>
