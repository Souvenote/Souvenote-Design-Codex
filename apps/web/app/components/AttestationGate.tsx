    ...
    <div className="attg-wrap" role="dialog" aria-modal="true" data-screen-label="Modal · Image-rights attestation">
      ...
      <div className="attg-modal">
        <div className="attg-head">
          <div className="attg-eyebrow">
            <AttgIcon name="lock" w={13} />
            <span>Required before upload</span>
          </div>
          <h2 className="attg-title">Review &amp; confirm before we continue.</h2>
          <p className="attg-sub">
            Because your upload contains a real person&rsquo;s likeness, please read both documents in full, then
            confirm the statements below.
          </p>
          <button type="button" className="attg-close" onClick={onClose} aria-label="Close">
            <AttgIcon name="close" w={15} />
          </button>
        </div>
        ...
        <div className="attg-tabs">
          {(Object.keys(DOCS) as LegalDocKey[]).map((k, i) => (
            <React.Fragment key={k}>
              {i > 0 && (
                <span className="attg-tab-arrow">
                  <AttgIcon name="arrow" w={16} />
                </span>
              )}
              <div className={`attg-tab ${k === activeDoc ? 'is-active' : ''} ${read[k] ? 'is-read' : ''}`}>
                <span className="attg-tab-dot">
                  <AttgIcon name="check" w={12} />
                </span>
        ...
        <div className="attg-reader" ref={readerRef} onScroll={onScroll}>
          ...
          <div className="attg-docdivider" ref={privacyRef}>
            <span>
              <AttgIcon name="check" w={13} /> End of Terms &mdash; Privacy Policy next
            </span>
          </div>
          ...
          <div className="attg-reader-end">
            <AttgIcon name="check" w={14} /> You&rsquo;ve reached the end of both documents.
          </div>
        </div>

        <div className="attg-progress">
          <i style={{ width: pct + '%' }} />
        </div>

        {!bothRead && (
          <div className="attg-scrollhint">
            <AttgIcon name="down" w={13} />{' '}
            {read.terms
              ? 'Keep scrolling to the end of the Privacy Policy to continue'
              : 'Keep scrolling \u2014 read both documents to the end to continue'}
          </div>
        )}

        <div className="attg-checks">
          {!bothRead ? (
            <div className="attg-locknote">
              <AttgIcon name="lock" w={13} /> Read both the Terms of Service and Privacy Policy to the end to unlock the
              confirmations below.
            </div>
          ) : (
            <React.Fragment>
              <label className={`attg-check is-enabled ${a1 ? 'is-checked' : ''} ${showErr && !a1 ? 'is-err' : ''}`}>
                <span className="attg-check-box">
                  <AttgIcon name="check" w={13} />
                </span>
                <input
                  type="checkbox"
                  checked={a1}
                ...
                />
                <span className="attg-check-text">
                  I certify that I have the explicit consent of the person depicted to upload their photo and process
                  their likeness through this app, and that I own the copyright to the photo.
                </span>
              </label>
              <label className={`attg-check is-enabled ${a2 ? 'is-checked' : ''} ${showErr && !a2 ? 'is-err' : ''}`}>
                ...
                />
                <span className="attg-check-text">
                  I agree to the Terms of Service and Privacy Policy, including guidelines on acceptable use, data
                  retention, and the Intimate Images Protection Act.
                </span>
              </label>
            </React.Fragment>
          )}
        ...
        <div className="attg-foot">
          <span className="attg-foot-note">
            {bothRead
              ? bothChecked
                ? 'Thank you \u2014 you\u2019re all set.'
                : 'Confirm both statements to proceed.'
              : 'Both documents must be read in full.'}
          </span>
          <div style={{ display: 'flex', gap: 10 }}>
            <button type="button" className="attg-btn attg-btn-secondary" onClick={onClose}>
              Cancel
            </button>
