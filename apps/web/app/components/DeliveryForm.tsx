...
function dlvValidate(recipient: DeliveryRecipient): DeliveryErrors {
  ...
  if (pc && !errs.postalCode) {
    const ok =
      recipient.country === 'US' ? /^\d{5}(-\d{4})?$/.test(pc) : /^[A-Za-z]\d[A-Za-z]\s?\d[A-Za-z]\d$/.test(pc);
...
function DlvAddressFields({ value, onChange, errors = {}, compact = false }: DlvAddressFieldsProps) {
  ...
  return (
    <div className="dlv-fgrid" style={{ gap: 16 }}>
      <div className="dlv-fgrid title-name">
        ...
        <div className="dlv-field">
          <label className="bmc-label">
            First name<span className="dlv-req">*</span>
          </label>
          <input
            className={`dlv-input ${err('firstName')}`}
        ...
        <div className="dlv-field">
          <label className="bmc-label">
            Last name<span className="dlv-req">*</span>
          </label>
          <input
            className={`dlv-input ${err('lastName')}`}
      ...
      <div className="dlv-field">
        <label className="bmc-label">
          Company <span className="bmc-opt">(optional)</span>
        </label>
        <input
          className="dlv-input"
          value={value.company}
          onChange={set('company')}
          placeholder="Add a company or 'c/o' line"
        />
      </div>

      <div className="dlv-field">
        <label className="bmc-label">
          Address line 1<span className="dlv-req">*</span>
        </label>
        <input
          className={`dlv-input ${err('address1')}`}
      ...
      {!compact && (
        <div className="dlv-fgrid cols-2">
          <div className="dlv-field">
            <label className="bmc-label">
              Address line 2 <span className="bmc-opt">(optional)</span>
            </label>
            <input
              className="dlv-input"
          ...
          <div className="dlv-field">
            <label className="bmc-label">
              Address line 3 <span className="bmc-opt">(optional)</span>
            </label>
            <input
              className="dlv-input"
      ...
      <div className="dlv-fgrid cols-3">
        <div className="dlv-field">
          <label className="bmc-label">
            City<span className="dlv-req">*</span>
          </label>
          <input
            className={`dlv-input ${err('city')}`}
            value={value.city}
            onChange={set('city')}
        ...
        <div className="dlv-field">
          <label className="bmc-label">
            {c.region}
            <span className="dlv-req">*</span>
          </label>
          <input
            className={`dlv-input ${err('state')}`}
            value={value.state}
            onChange={set('state')}
        ...
        <div className="dlv-field">
          ...
          <input
            className={`dlv-input ${err('postalCode')}`}
    ...
    <div className="bmc-card dlv-section">
      <div className="dlv-section-head">
        ...
        <div className="bmc-chip-row">
          <button
            type="button"
            className={`bmc-chip ${mode === 'single' ? 'is-active' : ''}`}
          ...
          <button
            type="button"
            className={`bmc-chip ${mode === 'multiple' ? 'is-active' : ''}`}
          ...
          {recipients.map((recipient, index) => (
            <div key={index} className={`dlv-recip-item ${editingIdx === index ? 'is-editing' : ''}`}>
              ...
              <div className="dlv-recip-acts">
                <button type="button" className="dlv-icon-btn" onClick={() => editRecip(index)} aria-label="Edit">
                  <BmcIcon name="edit" w={15} />
                </button>
                <button type="button" className="dlv-icon-btn" onClick={() => removeRecip(index)} aria-label="Remove">
                  <BmcIcon name="close" w={15} />
                </button>
              </div>
            </div>
          ))}
    ...
    <div className="bmc-card dlv-section">
      <div className="dlv-section-head">
        ...
        <div className="bmc-chip-row">
          <button
            type="button"
            className={`bmc-chip ${when === 'now' ? 'is-active' : ''}`}
            onClick={() => setWhen('now')}
          >
            Send now
          </button>
          <button
            type="button"
            className={`bmc-chip ${when === 'schedule' ? 'is-active' : ''}`}
