  ...
  return (
    <>
      <PageHero
        crumbs={crumbs}
        title={title}
        meta={<>Last updated {"\u00b7"} <b>{lastUpdated}</b></>}
      />
        ...
        <div className="pg-doc">
          ...
          {sections.map((section, index) => (
            <section className="pg-section" id={section.id} key={section.id}>
              ...
              <h2>{section.title}</h2>
              {section.paras.map((paragraph, paragraphIndex) => <p key={paragraphIndex}>{paragraph}</p>)}
              {section.bullets && (
...
function ContactPage() {
  ...
  const cards: ContactCard[] = [
    { ico: <IconMail />, title: "Email Us", body: <>Reach the team directly at <a href="mailto:hello@souvenote.com">hello@souvenote.com</a>.</> },
  ...
  return (
    <>
      <PageHero
        ...
        title={<>Get in <span className="pg-italic text-metallic-rose-gold">touch</span></>}
      ...
      <div className="pg-contact">
        ...
        >
          ...
          <button type="submit" className="btn-matte">{sent ? "Message sent \u2713" : "Send Message"}</button>
