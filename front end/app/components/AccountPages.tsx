...
const AccIco = {
  ...
  gift: (
    ...
    >
      ...
      <path d="M3 9V8a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v1M12 6v15M12 6S10.5 3 8.5 3 5.5 5 7 6h5M12 6s1.5-3 3.5-3 2.5 2 1 3h-5" />
    </svg>
  ),
  cal: (
    <svg
  ...
  users: (
    ...
    >
      ...
      <path d="M16 5.2a3.2 3.2 0 0 1 0 6.1M17.5 20a5.5 5.5 0 0 0-2.5-4.6" />
    </svg>
  ),
  chev: (
    <svg
...
function isApprovedProfileAsset(asset: CardDraftAsset) {
  const moderationState = String(
    asset.moderationState || asset.moderation_state || "",
  ).toLowerCase();
  const approvedAt = asset.approvedAt || asset.approved_at;
  return (
    Boolean(approvedAt) &&
    ["approved", "approved_mock"].includes(moderationState)
  );
}

function profileAssetType(asset: CardDraftAsset) {
  return String(asset.assetType || asset.asset_type || "").toLowerCase();
}

function profileOrderStatus(status: string) {
  switch (status) {
    case "pending":
      return "is ready for checkout";
    case "checkout_started":
      return "has checkout in progress";
    case "payment_authorized":
      return "is waiting for send confirmation";
    case "paid":
    case "paid_mock":
      return "is paid and ready for fulfillment";
    case "fulfillment_started":
    case "fulfillment_submitted":
      return "was submitted for production";
    case "printing":
      return "is being produced";
    case "shipped":
      return "has shipped";
    case "delivered":
      return "was delivered";
    case "fulfilled_mock":
      return "completed in local fulfillment";
    case "closed_no_send":
      return "was closed without sending";
    case "fulfillment_on_hold":
    case "fulfillment_failed":
    case "failed_mock":
      return "needs fulfillment attention";
    case "payment_failed":
    case "payment_canceled":
    case "checkout_expired":
      return "needs checkout attention";
    default:
      return "was updated";
  }
}

function profileActivityTime(value: string | null | undefined) {
...
function ProfilePage({ user }: AccountPageProps) {
  ...
  React.useEffect(() => {
    ...
    if (!isAuthenticated || !localUserId) {
      setCreationCounts({ cards: 0, songs: 0 });
    ...
    fetchUserCardDrafts()
      .then(async (drafts) => {
        const assetGroups = await Promise.all(
          drafts.map((draft) => fetchCardDraftAssets(draft.id)),
        );
        const approvedAssetGroups = assetGroups.map((assets) =>
          assets.filter(isApprovedProfileAsset),
        );
        const cards = approvedAssetGroups.filter((assets) =>
          assets.some((asset) => profileAssetType(asset) === "image"),
        ).length;
        const songs = approvedAssetGroups.reduce(
          (total, assets) =>
            total +
            assets.filter((asset) => profileAssetType(asset) === "song").length,
          0,
        );
        if (!active) return;
        setCreationCounts({ cards, songs });
        setCreationStatus("ready");
      })
      .catch(() => {
        if (!active) return;
  ...
  const profileStats: ProfileStat[] = [
    {
      ...
      label: creditBalance.status === "error" ? "Credits offline" : "Credits",
      gold: true,
    },
    {
      num: creationStatus === "loading" ? "..." : String(creationCounts.cards),
      label: creationStatus === "error" ? "Cards offline" : "Cards made",
    },
    {
  ...
  const orderActivity: ProfileActivity[] = recentOrders.map((order) => ({
    ico: AccIco.send,
    title: (
      <>
        Order <b>{order.id.slice(0, 8).toUpperCase()}</b>{" "}
        {profileOrderStatus(order.status)}
      </>
    ),
    time: profileActivityTime(order.updatedAt || order.createdAt),
  }));
      ...
      ? [
          {
            ...
            title: (
              <>
                Bought{" "}
                <b>
                  {blankGiftCount} blank Souvenote{" "}
                  {blankGiftCount === 1 ? "gift" : "gifts"}
                </b>
              </>
            ),
            time: "Ready now",
    ...
    <div className="bmc-shell" data-screen-label="Profile">
      <div className="bmc-head" style={{ marginBottom: 28 }}>
        ...
        <h1 className="bmc-title">
          Your{" "}
          <span className="souv-hero-italic text-metallic-gold">profile</span>
        </h1>
        <p className="bmc-lede">
          Everything about your account at a glance: credits, what you&apos;ve
          made, and quick ways back into the things you do most.
        </p>
      </div>
      ...
      <div className="acc-hero">
        ...
        <div className="acc-hero-actions">
          <Link className="bmc-cta-secondary" href="/account/settings">
            {AccIco.edit} Edit profile
          </Link>
        </div>
      </div>
      ...
      <div className="acc-stats">
        {profileStats.map((stat) => (
          <div className="acc-stat" key={stat.label}>
            <div className={`acc-stat-num ${stat.gold ? "is-gold" : ""}`}>
              {stat.num}
            </div>
        ...
        <div className="acc-panel acc-profile-gift-card">
          <div className="acc-profile-gift-copy">
            ...
            <h2 className="acc-profile-gift-title">
              {blankGiftCount} blank{" "}
              {blankGiftCount === 1 ? "Souvenote" : "Souvenotes"} ready to give
            </h2>
            <p>
              This blank gift also appears in Saved Cards &amp; Songs. When you
              send a card, we&apos;ll remind you to choose who should receive it
              or keep it saved for later.
            </p>
          </div>
          <div className="acc-profile-gift-summary">
            ...
            <Link
              className="bmc-cta acc-profile-gift-cta"
              href="/create/my-cards-and-songs"
            >
          ...
          <div className="acc-activity">
            {profileActivity.length ? (
              ...
              <div className="acc-activity-row">
                ...
                <div className="acc-activity-main">
                  <div className="acc-activity-title">
                    {ordersStatus === "error"
                      ? "Recent order activity is temporarily unavailable."
                      : ordersStatus === "loading"
                        ? "Loading recent order activity..."
                        : "No activity yet. Create your first card to get started."}
                  </div>
                </div>
                <div className="acc-activity-time">
                  {ordersStatus === "error"
                    ? "Offline"
                    : ordersStatus === "loading"
                      ? "..."
...
const REFER_STEPS: ReferStep[] = [
  {
    n: 1,
    h: "Share your link",
    p: "Send your personal invite link to friends by text, email or social.",
  },
  {
    n: 2,
    h: "They make a card",
    p: "Your friend signs up and creates their first Souvenote card.",
  },
  {
    n: 3,
    h: "You both earn",
    p: "They get 10 credits to start, and you get 10 credits dropped in your account.",
  },
];
...
const REFER_SHARE: ReferShare[] = [
  ...
  {
    label: "Facebook",
    path: "M22 12a10 10 0 1 0-11.6 9.9v-7H8v-2.9h2.5V9.7c0-2.5 1.5-3.9 3.8-3.9 1.1 0 2.2.2 2.2.2v2.5H15c-1.2 0-1.6.8-1.6 1.6V12h2.7l-.4 2.9h-2.3v7A10 10 0 0 0 22 12z",
  },
];
...
function ReferPage({ user }: AccountPageProps) {
  ...
  return (
    <div className="bmc-shell" data-screen-label="Refer a Friend">
      <div
        className="bmc-head"
        style={{ marginBottom: 36, maxWidth: "100%", textAlign: "center" }}
      >
      ...
      <div className="acc-refer-hero">
        <div className="acc-refer-reward">
          <span className="amt is-shimmer">Give 10</span>
          <span className="x">{"\u00b7"}</span>
          <span className="amt">Get 10</span>
        </div>
        <h1
          className="bmc-title"
          style={{ textAlign: "center", margin: "6px 0 12px" }}
        >
          Share the{" "}
          <span className="souv-hero-italic text-metallic-rose-gold">love</span>
        </h1>
        <p className="bmc-lede" style={{ margin: "0 auto" }}>
          Invite a friend to Souvenote. They get 10 credits to start, and you
          earn 10 credits the moment they send a card.
        </p>
      </div>
      ...
      <div
        className="acc-panel acc-referlink-wrap"
        style={{ marginBottom: 22 }}
      >
        <div
          className="acc-flabel"
        ...
        <div className="acc-share-row">
          ...
          {REFER_SHARE.map((share) => (
            ...
            >
              <svg
                viewBox="0 0 24 24"
                fill={share.label === "Email" ? "none" : "currentColor"}
                stroke={share.label === "Email" ? "currentColor" : "none"}
                strokeWidth="1.7"
                strokeLinecap="round"
                strokeLinejoin="round"
              >
      ...
      <div className="acc-grid-2">
        ...
        >
          <div
            className="acc-panel-title"
            style={{ borderBottom: 0, marginBottom: 6 }}
          >
            Credits earned
          </div>
