...
const PAGE_HREFS = {
  ...
  'Account Settings': '#todo-account-settings',
  'About': '#todo-about',
  'Contact': '#todo-contact',
  'FAQ': '#todo-faq',
  'Terms of Service': '#todo-terms',
  'Privacy Policy': '#todo-privacy',
  'Cookie Policy': '#todo-cookie-policy',
  'Refund Policy': '#todo-refund-policy',
};

const FOOTER_COLS = [
  { title: 'Create',    items: ['Build My Card', 'Personalize a Template', 'Community Cards', 'My Cards & Songs'] },
      ...
      <div className="souv-footer-inner">
        <div className="souv-footer-brand">
          ...
          <div className="souv-footer-social">
            {SOCIAL.map(s => (
              <a key={s.label} className="souv-footer-social-btn" aria-label={s.label} href="#todo-social">
        ...
        <div className="souv-footer-cols">
          {FOOTER_COLS.map(col => (
            <div key={col.title} className={`souv-footer-col ${col.title === 'Company' ? 'souv-footer-col-2up' : ''}`}>
              ...
              <ul>
                {col.items.map(i => <li key={i}><Link href={PAGE_HREFS[i] || '#todo'} className="souv-footer-link">{i}</Link></li>)}
              </ul>
            </div>
          ))}
        </div>
